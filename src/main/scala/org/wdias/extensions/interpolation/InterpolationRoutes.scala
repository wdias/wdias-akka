...
// Interpolation REST API Routes class
trait InterpolationRoutes extends Protocols {
  // abstract system value will be provide by app
  implicit def system: ActorSystem
  implicit def materializer: ActorMaterializer

  // logging for InterpolationRoutes
  lazy val logInterpolationRoutes = Logging(system, classOf[InterpolationRoutes])

  // Actor dependencies required for InterpolationRoutes
  def extensionAdapterRef: ActorRef

  // Required by the `ask` (?) method below
  implicit lazy val timeoutInterpolationRoutes: Timeout = Timeout(15.seconds) // TODO: Obtain from config

  // --- All Input Routes ---
  lazy val interpolationRoutes: Route = {
    concat(
      pathPrefix("location" / "point") {
        concat(
          // GET: Get Point
          (get & pathPrefix(Segment)) { locationId: String =>
            logInterpolationRoutes.info("/location/point GET request: > {}", locationId)
          ...
          (get & pathEnd & parameters('locationId.as[String].?, 'name.as[String].?)) { (locationId, name) =>
            logInterpolationRoutes.info("/location/point GET request: List")
          ...
          (post & entity(as[Location])) { location: Location =>
            logInterpolationRoutes.info("/location/point POST request: > {}", location)
          ...
          (put & pathPrefix(Segment)) { locationId: String =>
            entity(as[Location]) { location: Location =>
              logInterpolationRoutes.info("/location/point GET request: Replace {} : {}", locationId, location)
          ...
          (patch & pathPrefix(Segment)) { locationId: String =>
            ...
            entity(as[Location]) { location: Location =>
              logInterpolationRoutes.info("/location/point GET request: Update {} : {}", locationId, location)
              complete(Created -> "Update")
            }
          },
          // DELETE: Delete Point
          (delete & pathPrefix(Segment)) { locationId: String =>
            logInterpolationRoutes.info("/location/point DELETE request: > {}", locationId)
